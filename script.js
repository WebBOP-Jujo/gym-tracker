...
/* --- Función para mostrar el estado de confirmación de reset --- */
const showResetConfirmation = (show) => {
    if (show) {
        ...
        /* authEmailInput?.setAttribute('disabled', 'true'); */ /* Opcional: deshabilitar email */
    } else {
        passwordInputContainer?.classList.remove('hidden');
        authButtonsContainer?.classList.remove('hidden');
        resetConfirmationMessage?.classList.add('hidden');
        /* authEmailInput?.removeAttribute('disabled'); */ /* Habilitar si se deshabilitó */
    }
};
...
/* --- AUTHENTICATION --- */
const handleAuthChange = (event, session) => { console.log("Auth:", event, "| Session:", !!session); switch (event) { case 'SIGNED_IN': if (session) { currentUser = session.user; setAuthState(false); if (authError) authError.textContent = ''; if (userEmailDisplay) userEmailDisplay.textContent = currentUser.email || 'Usuario'; initializeAppData(); } break; case 'SIGNED_OUT': currentUser = null; setAuthState(true); showResetConfirmation(false); /* Resetear la vista de auth al cerrar sesión */ masterExerciseList = []; workoutHistory = {}; loadedDatesSet = new Set(); exercisesWithHistory.clear(); if (chartInstance) { chartInstance.destroy(); chartInstance = null; } if (historyLog) historyLog.innerHTML = ''; if (historyCountSpan) historyCountSpan.textContent = '(Total: 0 días)'; clearWorkoutForm(); populateExerciseDropdowns(); populateGraphExerciseDropdown(); hideChart(); if (filterDateInput) filterDateInput.value = ''; break; case 'INITIAL_SESSION': console.log("INITIAL_SESSION event (handled by getSession)"); break; case 'USER_UPDATED': if (session) { currentUser = session.user; if (userEmailDisplay) userEmailDisplay.textContent = currentUser.email || 'Usuario'; } console.log("USER_UPDATED event"); break; case 'PASSWORD_RECOVERY': showToast('Enlace de recuperación enviado. Revisa tu email.', 'info', 6000); break; default: console.log("Unhandled Auth:", event); } };
...
const getInitialTheme = () => { let theme = 'dark'; /* Predeterminado oscuro */ try { const stored = localStorage.getItem(THEME_STORAGE_KEY); if (stored === 'light') { /* Solo cambia si 'light' está guardado */ theme = 'light'; } } catch (e) { console.error("Failed to read theme pref, defaulting to dark:", e); } console.log(`Initial theme determined: ${theme}`); return theme; };
...
/* === LISTENER RESTABLECER CONTRASEÑA (MEJORADO) === */
forgotPasswordLink?.addEventListener('click', async (e) => {
    ...
    if (!email || !/\S+@\S+\.\S+/.test(email)) {
        if (authError) authError.textContent = "Por favor, introduce un email válido.";
        authEmailInput?.focus();
        return;
    }
    if (!supabaseClient) {
         if (authError) authError.textContent = "Error de configuración.";
         console.error("Supabase client not available for password reset.");
         return;
    }

    showToast('Procesando solicitud...', 'info', 2000); /* Feedback rápido */
    try {
        const { error } = await supabaseClient.auth.resetPasswordForEmail(email, {
            /* redirectTo: 'URL_OPCIONAL' */
        });

        if (error) {
             /* Si es un error real (rate limit), lo lanzamos para el catch */
             if (error.message.includes("rate limit") || error.message.includes("For security purposes")) {
                throw error; /* Ir al catch para mostrar en #auth-error */
             } else {
                 /* Otros errores (como user not found) los tratamos como éxito por seguridad */
                 console.warn("Password Reset Issue (Supabase treated as success):", error.message);
                 showResetConfirmation(true); /* Mostrar el mensaje de confirmación */
             }
        } else {
            /* Éxito explícito desde Supabase */
            showResetConfirmation(true); /* Mostrar el mensaje de confirmación */
        }

    } catch (error) { /* Captura errores lanzados (ej. rate limit) */
        ...
        showResetConfirmation(false); /* Asegurarse de que el formulario normal está visible */
    }
});
/* === FIN LISTENER RESTABLECER CONTRASEÑA === */

/* === LISTENER PARA BOTÓN "VOLVER A INICIO DE SESIÓN" === */
backToLoginBtn?.addEventListener('click', () => {
    showResetConfirmation(false); /* Oculta mensaje y muestra formulario normal */
});
/* === FIN LISTENER VOLVER === */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!supabaseClient) { console.error("Supabase client NOT defined!"); alert("Critical Error: Supabase client failed to initialize. Check console."); document.body.innerHTML = '<p style="color: red; padding: 20px;">Error crítico al cargar la aplicación.</p>'; return; }
    ...
    supabaseClient.auth.getSession().then(({ data: { session } }) => {
        ...
        if (session) { currentUser = session.user; setAuthState(false); if (userEmailDisplay) userEmailDisplay.textContent = currentUser.email || 'Usuario'; initializeAppData(); }
        else { setAuthState(true); }
        /* Asegurar que el formulario de reset no se muestre inicialmente */
        showResetConfirmation(false);
        console.log("Initial UI state set based on session.");
    }).catch(error => {
        console.error("Error during initial getSession:", error);
        setAuthState(true);
        showResetConfirmation(false); /* También aquí por si acaso */
    });
