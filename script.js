...
let workoutHistory = {}; // { "DD/MM/YYYY": [entry1, entry2], ... }
let loadedDatesSet = new Set();
let currentUser = null;
let chartInstance = null;
let exercisesWithHistory = new Set(); // Set of exercise names with data suitable for graphing
...
const authError = document.getElementById('auth-error'); // Keep for login/signup specific errors
...
const notificationArea = document.getElementById('notification-area'); // Reference to the toast container
...
const showToast = (message, type = 'info', duration = 3500) => {
    if (!notificationArea) {
        console.error("Elemento #notification-area no encontrado para toast.");
        console.warn(`Fallback Toast (${type}): ${message}`); // Log to console as fallback
        return;
    }

    const toast = document.createElement('div');
    toast.classList.add('toast-notification', type); // e.g., 'success', 'error', 'info'
    ...
    // Allow user to dismiss by clicking the toast
    toast.addEventListener('click', () => {
        clearTimeout(timeoutId); // Cancel the auto-dismiss timer
        toast.classList.add('fade-out'); // Start fade-out immediately
        // The existing animationend listener will handle removal
    }, { once: true }); // Listener fires only once per toast
};

// REMOVED: displayStatusMessage
// REMOVED: displayInlineStatus
...
const updateWorkoutSetInDB = async (entryId, reps, weight) => {
    if (!currentUser || !entryId || !supabaseClient || !saveEditBtn) return; if ((reps !== null && (isNaN(reps) || reps < 0)) || (weight !== null && (isNaN(weight) || weight < 0))) { showToast('Valores inválidos.', 'error'); /* Use toast instead of inline status */ return; } showLoading(saveEditBtn, true); try { const { data, error } = await supabaseClient.from('Entrenamientos').update({ "Reps": reps, "Weight": weight }).eq('id', entryId).select().single(); if (error) { console.error("Error updating set:", error); const errorMsg = error.code === '42501' ? "Error permisos." : `Error DB: ${error.message}`; showToast(errorMsg, 'error'); /* Use toast */ return; } showToast('Serie actualizada.', 'success'); /* Use toast */ let updatedLocally = false; let dateKeyOfUpdatedEntry = null; for (const dateKey in workoutHistory) { const entryIndex = workoutHistory[dateKey].findIndex(entry => entry.id === entryId); if (entryIndex > -1) { if (data) { workoutHistory[dateKey][entryIndex] = data; } else { workoutHistory[dateKey][entryIndex].Reps = reps; workoutHistory[dateKey][entryIndex].Weight = weight; } updatedLocally = true; dateKeyOfUpdatedEntry = dateKey; break; } } if (updatedLocally) { const currentFilterDateVal = filterDateInput?.value; const currentFormattedFilterDate = currentFilterDateVal ? formatDate(new Date(currentFilterDateVal + 'T00:00:00Z')) : null; if (!currentFormattedFilterDate || currentFormattedFilterDate === dateKeyOfUpdatedEntry) { if (currentFormattedFilterDate) { displayWorkoutHistory(filterSpecificDate(currentFormattedFilterDate)); } else { displayWorkoutHistory(getRecentHistorySubset()); } } await updateGraphDropdownAndChart(); } setTimeout(closeEditModal, 1000); } catch (error) { console.error('Unexpected error updating:', error); showToast(`Error: ${error.message}`, 'error'); /* Use toast */ } finally { showLoading(saveEditBtn, false); }
};
...
const updateChartData = async () => {
    if (!graphExerciseSelect /*REMOVED chartStatus*/ || !progressChartCanvas) { console.warn("Graph elements missing."); hideChart(); return; } const selectedExercise = graphExerciseSelect.value; if (!selectedExercise) { hideChart(); return; } showToast('Calculando datos gráfica...', 'info', 2000); /* Temp info toast */ const dailyAvgE1RM = {}; for (const dateKey in workoutHistory) { const entriesForDate = workoutHistory[dateKey] || []; for (const entry of entriesForDate) { if (entry && entry.Exercise === selectedExercise && entry.Weight != null && entry.Reps != null && entry.Reps > 0 && entry.Weight > 0) { const e1RM = calculateE1RM(entry.Weight, entry.Reps); if (e1RM > 0) { const dateObj = parseDate(entry.Date); if (!dateObj) continue; const dateKeyISO = dateObj.toISOString().split('T')[0]; if (!dailyAvgE1RM[dateKeyISO]) { dailyAvgE1RM[dateKeyISO] = { totalE1RM: 0, count: 0 }; } dailyAvgE1RM[dateKeyISO].totalE1RM += e1RM; dailyAvgE1RM[dateKeyISO].count++; } } } } const sortedDatesISO = Object.keys(dailyAvgE1RM).sort();
    if (sortedDatesISO.length < 2) { console.warn(`updateChartData called for ${selectedExercise} with ${sortedDatesISO.length} points.`); if (chartInstance) { chartInstance.destroy(); chartInstance = null; } const ctx = progressChartCanvas; if (ctx) { ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height); } /* Error toast shown by button handler */ return; } const labels = sortedDatesISO.map(dateISO => formatDate(dateISO)); const dataPoints = sortedDatesISO.map(dateISO => (dailyAvgE1RM[dateISO].totalE1RM / dailyAvgE1RM[dateISO].count).toFixed(1)); renderProgressChart(labels, dataPoints, selectedExercise); /* Clear status toast not needed */
};
...
const handleAuthChange = (event, session) => {
    console.log("Auth Event:", event, "| Session:", session ? session.user.email : 'No'); switch (event) { case 'SIGNED_IN': if (session) { console.log("Signed In:", session.user.email); currentUser = session.user; if (authSection) authSection.classList.add('hidden'); if (appSection) appSection.classList.remove('hidden'); if (authError) authError.textContent = ''; initializeAppData(); } break; case 'SIGNED_OUT': console.log("Signed Out"); currentUser = null; if (authSection) authSection.classList.remove('hidden'); if (appSection) appSection.classList.add('hidden'); masterExerciseList = []; workoutHistory = {}; loadedDatesSet = new Set(); exercisesWithHistory.clear(); if (chartInstance) { chartInstance.destroy(); chartInstance = null; } if (historyLog) historyLog.innerHTML = ''; if (historyCountSpan) historyCountSpan.textContent = '(Total: 0 días)'; clearWorkoutForm(); populateExerciseDropdowns(); populateGraphExerciseDropdown(); hideChart(); if (filterDateInput) filterDateInput.value = ''; break; case 'INITIAL_SESSION': console.log("Initial Session:", session ? "Found" : "None"); break; case 'USER_UPDATED': if (session) currentUser = session.user; console.log("User Updated:", session?.user?.email); break; case 'PASSWORD_RECOVERY': console.log("Password Recovery Started."); break; default: console.log("Unhandled Auth Event:", event); }
};
...
// MODIFIED Workout Form Submit Listener (using HTML5 Validation API)
workoutForm?.addEventListener('submit', (e) => {
    ...
    if (!workoutData.date) { showToast('Fecha inválida.', 'error'); workoutDateInput.focus(); return; } // Use toast
    if (!workoutData.exercise) { showToast('Selecciona un ejercicio.', 'error'); exerciseSelect.focus(); return; } // Use toast
    const setRows = setsContainer.querySelectorAll('.set-row');
    if (setRows.length === 0) { showToast('Añade al menos una serie.', 'error'); return; } // Use toast

    let isFormCompletelyValid = true;
    for (const row of setRows) {
        const repsInput = row.querySelector('.reps-input');
        const weightInput = row.querySelector('.weight-input');
        repsInput.setCustomValidity(""); weightInput.setCustomValidity(""); // Clear previous custom errors
        ...
        if (isNaN(weightValue) || weightValue <= 0) { weightInput.setCustomValidity("El peso debe ser mayor que 0."); weightInput.reportValidity(); weightInput.setCustomValidity(""); isFormCompletelyValid = false; break; }
    ...
    else { console.log("Formulario inválido, no se guarda."); /* Browser shows bubble */ }
});

toggleManageBtn?.addEventListener('click', () => { if (!exerciseManagementSection || !toggleManageBtn) return; const section = exerciseManagementSection; const icon = toggleManageBtn.querySelector('i'); section.classList.toggle('hidden'); if (icon) { icon.className = section.classList.contains('hidden') ? 'fas fa-list-ul' : 'fas fa-eye-slash'; } /* REMOVED manageExerciseStatus */ });
...
// MODIFIED Graph Button Listener (uses hasEnoughDataForChart check first)
showSelectedGraphBtn?.addEventListener('click', () => {
    // REMOVED chartStatus reference
    if (!graphExerciseSelect) return;
    const selectedExercise = graphExerciseSelect.value;
    if (!selectedExercise) {
        showToast('Selecciona un ejercicio para ver la gráfica.', 'info'); // Use toast
        if (chartContainer && !chartContainer.classList.contains('hidden')) hideChart();
        return;
    }

    if (hasEnoughDataForChart(selectedExercise)) {
        console.log(`Showing graph for ${selectedExercise}`);
        if (chartContainer) chartContainer.classList.remove('hidden');
        if (hideGraphBtn) hideGraphBtn.classList.remove('hidden');
        if (showSelectedGraphBtn) showSelectedGraphBtn.classList.add('hidden');
        updateChartData();
    } else {
        console.log(`Not enough data for graph: ${selectedExercise}`);
        showToast(`Se necesitan al menos 2 días para graficar '${selectedExercise}'.`, 'info'); // Use toast
        // Do NOT change UI state (buttons/container)
    }
});

hideGraphBtn?.addEventListener('click', () => { console.log("Hiding graph"); hideChart(); });
graphExerciseSelect?.addEventListener('change', () => { console.log("Graph selection changed"); hideChart(); if (showSelectedGraphBtn) { showSelectedGraphBtn.disabled = !graphExerciseSelect.value; } /* REMOVED chartStatus */ });
editForm?.addEventListener('submit', (e) => {
    e.preventDefault(); if (!editEntryIdInput || !editRepsInput || !editWeightInput) return; const entryId = parseInt(editEntryIdInput.value, 10); const repsValue = editRepsInput.value.trim(); const weightValue = editWeightInput.value.trim(); const reps = repsValue === '' ? null : parseInt(repsValue, 10); const weight = weightValue === '' ? null : parseFloat(weightValue); if (!entryId || isNaN(entryId)) { showToast('Error: ID inválido.', 'error'); /* Use toast */ return; } if ((reps !== null && (isNaN(reps) || reps < 0)) || (weight !== null && (isNaN(weight) || weight < 0))) { showToast('Valores inválidos.', 'error'); /* Use toast */ return; } updateWorkoutSetInDB(entryId, reps, weight);
});
cancelEditBtns?.forEach(btn => { btn.addEventListener('click', closeEditModal); });

// --- INICIALIZACIÓN ---
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM loaded. Initializing..."); if (!supabaseClient) { console.error("Supabase client NOT defined!"); alert("Critical Error. Check console."); if(appSection) appSection.innerHTML = '<p class="error-message">Critical Error.</p>'; if(authSection) authSection.classList.add('hidden'); return; } if (!progressChartCanvas) { console.warn("Canvas #progress-chart not found."); } console.log("Setting up onAuthStateChange..."); supabaseClient.auth.onAuthStateChange(handleAuthChange); supabaseClient.auth.getSession().then(({ data: { session } }) => { console.log("Initial getSession:", session ? `OK for ${session.user.email}` : 'No session'); if (!session) { if (authSection) authSection.classList.remove('hidden'); if (appSection) appSection.classList.add('hidden'); console.log("Showing auth."); } }).catch(error => { console.error("Error getSession:", error); if (authSection) authSection.classList.remove('hidden'); if (appSection) appSection.classList.add('hidden'); }); console.log("Listeners/session init complete.");
});