...
const prefillFormBasedOnHistory = (exerciseName) => {
    if (!exerciseName || Object.keys(workoutHistory).length === 0) { if (numSetsInput) numSetsInput.value = 1; updateSetsUI(1); const firstRow = setsContainer?.querySelector('.set-row'); if (firstRow) { const repsInput = firstRow.querySelector('.reps-input'); const weightInput = firstRow.querySelector('.weight-input'); if (repsInput) { repsInput.value = ''; repsInput.placeholder = 'Reps'; } if (weightInput) { weightInput.value = ''; weightInput.placeholder = 'Peso'; } } return; }
